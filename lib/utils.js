...
const NPS_PREFIX = `${colors.hex("#87E752").bold("N")}${colors.blue.bold(
  "PS"
)}${colors.hex("#87E752").bold(">")}`;
const logger = {
...
const isEnclosedWithQuotes = val =>
...
const convertToPSParam = val => {
  switch (getType(val)) {
    case "String":
      return !isEnclosedWithQuotes(val) && (isIncludesSpaces(val) || isXML(val))
        ? `"${val}"`
        : val;
    case "Number":
      return val;
...
const isCriticalPSError = err =>
  CRITICAL_ERRORS.some(cErr => err.includes(cErr));
