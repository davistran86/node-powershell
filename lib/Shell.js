...
class Shell extends EventEmitter {
  constructor(options = {}) {
   ...
   https://rannn505.gitbook.io/node-powershell/shell/initialize`);
    ...
    proc.once("close", code => {
      ...
      this.emit("end", code);
      this.verbose &&
        logger.info(`PS process ${this.pid} exited with code ${code}`);
  ...
  addArgument(argument) {
    return new Promise((resolve, reject) => {
      if (this.commands.length === 0) {
        return reject(
          new PS_ARG_MISS_ERROR(
            "Commands array is empty. please add at least one command before you use addArgument()"
          )
        );
      }
  ...
  addParameter(parameter) {
    return new Promise((resolve, reject) => {
      if (this.commands.length === 0) {
        return reject(
          new PS_ARG_MISS_ERROR(
            "Commands array is empty. please add at least one command before you use addParameter()"
          )
        );
      }
      let lastCommand = this.commands.pop();
  ...
  addParameters(parameters = []) {
    return new Promise((resolve, reject) => {
      ...
      return resolve(
        Promise.all(parameters.map(this.addParameter, this)).then(
          () => this.commands
        )
      );
    });
  ...
  invoke() {
    ...
    const invocationCommands = this.commands
      .map(psCommand => psCommand.command)
      .join("; ");
    ...
    shellSafeWrite(this.streams.stdin, invocationCommands)
      ...
      // .then(() => shellSafeWrite(this.streams.stdin, os.EOL))
      .then(() =>
        shellSafeWrite(this.streams.stdin, `[Console]::Out.Write("${EOI}")`)
      )
      .then(() => shellSafeWrite(this.streams.stdin, os.EOL));
    ...
    return new Promise((resolve, reject) => {
      psOut.once("EOI", () => {
        ...
        if (invocationHadErrors) {
          this.invocationStateInfo =
            this.invocationStateInfo !== "Stopping" ? "Failed" : "Stopped";
        ...
        this.invocationStateInfo =
          this.invocationStateInfo !== "Stopping" ? "Completed" : "Stopped";
